...
public class ResortRoomFacilityGroupFilterRequest extends PaginatedRequest implements LocaleRequiredFilterable, LocaleSortable {
    ...
    public List<Predicate> toPredicates(Root<?> root, CriteriaQuery<?> query, CriteriaBuilder cb, Long localeId) {
        List<Predicate> predicates = SpecificationUtils.buildSearchPredicates(this, ResortRoomFacilityGroupSearchField.values(), root, query, cb, localeId);
    ...
    public LocaleJoinSortInfo getLocaleSortInfo(Long localeId) {
        return buildLocaleSortInfo("resortRoomFacilityGroupLocaleEntities", getSortBy(), getSortDir(), localeId, ResortRoomFacilityGroupSortField.localeSortFields());
    }
}
