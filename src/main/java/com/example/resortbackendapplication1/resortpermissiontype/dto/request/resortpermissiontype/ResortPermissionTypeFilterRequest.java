...
public class ResortPermissionTypeFilterRequest extends PaginatedRequest implements LocaleRequiredFilterable, LocaleSortable {
    ...
    public List<Predicate> toPredicates(Root<?> root, CriteriaQuery<?> query, CriteriaBuilder cb, Long localeId) {
        return SpecificationUtils.buildSearchPredicates(this, ResortPermissionTypeSearchField.values(), root, query, cb, localeId);
    }

    @Override
    public LocaleJoinSortInfo getLocaleSortInfo(Long localeId) {
        return buildLocaleSortInfo("resortPermissionTypeLocaleEntities", getSortBy(), getSortDir(), localeId, ResortPermissionTypeSortField.localeSortFields());
    }
}
