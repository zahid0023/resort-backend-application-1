...
public class PriceUnitFilterRequest extends PaginatedRequest implements LocaleRequiredFilterable, LocaleSortable {
    ...
    public List<Predicate> toPredicates(Root<?> root, CriteriaQuery<?> query, CriteriaBuilder cb, Long localeId) {
        List<Predicate> predicates = SpecificationUtils.buildSearchPredicates(this, PriceUnitSearchField.values(), root, query, cb, localeId);
        if (priceScopeCodes != null && !priceScopeCodes.isEmpty()) {
            ...
            Root<PriceUnitScopeAssignmentEntity> scopeAssignmentRoot = scopeAssignmentExists.from(PriceUnitScopeAssignmentEntity.class);
            scopeAssignmentExists.select(scopeAssignmentRoot.get("id"))
                    .where(
                            cb.equal(scopeAssignmentRoot.get("priceUnitEntity"), root),
