...
public class FacilityPriceTypeFilterRequest extends PaginatedRequest implements LocaleRequiredFilterable, LocaleSortable {
    ...
    public List<Predicate> toPredicates(Root<?> root, CriteriaQuery<?> query, CriteriaBuilder cb, Long localeId) {
        return SpecificationUtils.buildSearchPredicates(this, FacilityPriceTypeSearchField.values(), root, query, cb, localeId);
    }

    @Override
    public LocaleJoinSortInfo getLocaleSortInfo(Long localeId) {
        return buildLocaleSortInfo("facilityPriceTypeLocaleEntities", getSortBy(), getSortDir(), localeId, FacilityPriceTypeSortField.localeSortFields());
    }
}
