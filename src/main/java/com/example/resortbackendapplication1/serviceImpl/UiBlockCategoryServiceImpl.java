...
import com.example.resortbackendapplication1.model.mapper.UiBlockCategoryMapper;
import com.example.resortbackendapplication1.model.projection.UiBlockCategorySummary;
import com.example.resortbackendapplication1.repository.UiBlockCategoryRepository;
import com.example.resortbackendapplication1.service.UiBlockCategoryService;
...
public class UiBlockCategoryServiceImpl implements UiBlockCategoryService {
    ...
    public PaginatedResponse<UiBlockCategoryDto> getAllUiBlockCategories(Pageable pageable) {
        Page<@NonNull UiBlockCategorySummary> page = uiBlockCategoryRepository
                .findAllByIsActiveAndIsDeleted(true, false, pageable, UiBlockCategorySummary.class);

        Page<@NonNull UiBlockCategoryDto> dtoPage = page.map(p ->
                UiBlockCategoryDto.builder()
                        .id(p.getId())
                        .key(p.getKey())
                        .name(p.getName())
