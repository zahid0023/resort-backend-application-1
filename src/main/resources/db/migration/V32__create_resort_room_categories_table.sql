create table if not exists resort_room_categories
(
    id               bigserial primary key,

    resort_id        bigint                                             not null references resorts (id) on delete cascade,
    room_category_id bigint                                             not null references room_categories (id),

    code             varchar(50)                                        not null,
    sort_order       integer                  default 0                 not null,

    created_by       bigint                                             not null references users (id),
    created_at       timestamp with time zone default current_timestamp not null,
    updated_by       bigint                                             not null references users (id),
    updated_at       timestamp with time zone default current_timestamp not null,
    version          bigint                   default 0                 not null,
    is_active        boolean                  default true              not null,
    is_deleted       boolean                  default false             not null,
    deleted_by       bigint references users (id),
    deleted_at       timestamp with time zone,

    constraint uq_resort_room_category
        unique (resort_id, room_category_id),

    constraint uq_resort_room_category_code
        unique (resort_id, code)
);

create table resort_room_category_locales
(
    id                      bigserial primary key,

    resort_room_category_id bigint                                not null
        references resort_room_categories (id)
            on delete cascade,

    locale_id               bigint                                not null
        references locales (id),

    name                    varchar(150)                          not null,
    description             text        default ''                not null,
    sort_order              integer     default 0                 not null,

    created_by              bigint                                not null references users (id),
    created_at              timestamptz default current_timestamp not null,
    updated_by              bigint                                not null references users (id),
    updated_at              timestamptz default current_timestamp not null,
    version                 bigint      default 0                 not null,
    is_active               boolean     default true              not null,
    is_deleted              boolean     default false             not null,
    deleted_by              bigint references users (id),
    deleted_at              timestamptz,

    unique (resort_room_category_id, locale_id)
);

create table if not exists resort_room_category_metas
(
    id                      bigserial primary key,

    -- Room category this metadata belongs to.
    resort_room_category_id bigint                                not null
        references resort_room_categories (id)
            on delete cascade,

    ------------------------------------------------------------------------------
    -- OCCUPANCY
    ------------------------------------------------------------------------------

    max_adults              integer     default 2                 not null,
    max_children            integer     default 0                 not null,
    max_infants             integer     default 0                 not null,
    max_occupancy           integer     default 2                 not null,

    ------------------------------------------------------------------------------
    -- ROOM DETAILS
    ------------------------------------------------------------------------------

    room_size               numeric(10, 2),
    room_size_unit_id       bigint references units (id),

    bedroom_count           integer     default 1                 not null,
    bathroom_count          integer     default 1                 not null,

    ------------------------------------------------------------------------------
    -- CHECK-IN / CHECK-OUT
    ------------------------------------------------------------------------------

    default_check_in_time   time,
    default_check_out_time  time,

    ------------------------------------------------------------------------------
    -- EXTRA BED
    ------------------------------------------------------------------------------

    is_extra_bed_allowed    boolean     default false             not null,
    max_extra_beds          integer     default 0                 not null,

    ------------------------------------------------------------------------------
    -- POLICIES
    ------------------------------------------------------------------------------

    is_smoking_allowed      boolean     default false             not null,
    is_pets_allowed         boolean     default false             not null,

    ------------------------------------------------------------------------------
    -- BOOKING RULES
    ------------------------------------------------------------------------------

    minimum_stay_nights     integer     default 1                 not null,
    maximum_stay_nights     integer,

    ------------------------------------------------------------------------------
    -- AUDIT
    ------------------------------------------------------------------------------

    created_by              bigint                                not null
        references users (id),
    created_at              timestamptz default current_timestamp not null,
    updated_by              bigint                                not null
        references users (id),
    updated_at              timestamptz default current_timestamp not null,
    version                 bigint      default 0                 not null,
    is_active               boolean     default true              not null,
    is_deleted              boolean     default false             not null,
    deleted_by              bigint
        references users (id),
    deleted_at              timestamptz,

    ------------------------------------------------------------------------------
    -- CONSTRAINTS
    ------------------------------------------------------------------------------

    constraint uq_resort_room_category_meta
        unique (resort_room_category_id),

    constraint chk_meta_max_occupancy
        check (
            max_occupancy >=
            max_adults + max_children + max_infants
            ),

    constraint chk_meta_extra_beds
        check (max_extra_beds >= 0),

    constraint chk_meta_room_size
        check (
            room_size is null
                or room_size > 0
            ),

    constraint chk_meta_bedroom_count
        check (bedroom_count > 0),

    constraint chk_meta_bathroom_count
        check (bathroom_count > 0),

    constraint chk_meta_minimum_stay
        check (minimum_stay_nights > 0),

    constraint chk_meta_maximum_stay
        check (
            maximum_stay_nights is null
                or maximum_stay_nights >= minimum_stay_nights
            )
);

create table if not exists resort_room_category_beds
(
    id                      bigserial primary key,

    resort_room_category_id bigint                                not null
        references resort_room_categories (id)
            on delete cascade,

    bed_type_id             bigint                                not null
        references bed_types (id),

    quantity                integer     default 1                 not null,

    created_by              bigint                                not null
        references users (id),
    created_at              timestamptz default current_timestamp not null,
    updated_by              bigint                                not null
        references users (id),
    updated_at              timestamptz default current_timestamp not null,
    version                 bigint      default 0                 not null,
    is_active               boolean     default true              not null,
    is_deleted              boolean     default false             not null,
    deleted_by              bigint
        references users (id),
    deleted_at              timestamptz,

    constraint uq_resort_room_category_bed
        unique (resort_room_category_id, bed_type_id),

    constraint chk_bed_quantity
        check (quantity > 0)
);