...
Facility group scope assignments link a facility group to one or more facility scopes (e.g. `RESORT`, `ROOM_CATEGORY`,
`ROOM`), defining where that group's facilities can be applied. At least one scope must be provided when creating a
facility group. Additional scopes can be assigned or unassigned individually at any time.
...
The `scope_assignments` array is also embedded in the `GET /api/v1/facility-groups/{id}` response.
...
### Facility Group Scope Assignment (response)
...
Assigns a facility scope to the given facility group. If the scope was previously unassigned (soft-deleted), the
assignment is reactivated. If the scope is already actively assigned, a `409 CONFLICT` error is returned.
...
Returns the `facility_group_id` as `id`.
...
Returns all active scope assignments for the given facility group. Each entry includes the scope's code, sort order, and
all active locale translations.
...
```json
{
  ...
  "message": "FacilityGroupScopeAssignment not found for facilityGroupId: 1 and facilityScopeId: 99"
}
```
...
| 404         | `ENTITY_NOT_FOUND`      | Facility group or facility scope not found / deleted          |
