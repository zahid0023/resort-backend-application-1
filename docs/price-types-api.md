...
To fetch every price type usable in a given context (e.g. every price type a [Resort Facility
...
type with *any* active assignment to one of the given codes matches (multiple codes are OR'd together).
...
| `priceScopeCodes` | String[] | —               | must match an existing `price_scopes.code`        | Filter to price types assigned to any of the given price scope codes (e.g. `RESORT_FACILITY`) |
