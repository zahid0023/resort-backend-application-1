...
To fetch a facility group's facilities restricted to a given scope (e.g. every facility in the `RECREATION`
group that is `RESORT`-scoped), combine `facilityGroupId` with `facilityScopeCodes`:
`?facilityGroupId=3&facilityScopeCodes=RESORT`. `facilityScopeCodes` matches against `facility_scopes.code`
directly (e.g. `RESORT`, `ROOM_CATEGORY`, `ROOM`) so the frontend doesn't need to resolve a scope id first.
...
| `facilityScopeCodes` | String[] | —               | must match an existing `facility_scopes.code`                  | Filter to facilities assigned to any of the given facility scope codes (e.g. `RESORT`)                |
