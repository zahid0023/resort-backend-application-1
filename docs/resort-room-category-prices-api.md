...
**`BAS`/`WKD`/`WKE` rows cannot be created or deleted through this sub-resource — only `HOL`/`SPECIAL` can.**
`BAS`/`WKD`/`WKE` prices are created exclusively together with the resort room category itself, via the
required `prices` field on [Create Resort Room Category](resort-room-categories-api.md#create-resort-room-category),
and can never be deleted afterward — a room category must always keep exactly the BASE/WEEKDAY/WEEKEND rows it
was created with. [Create](#create-resort-room-category-price) and [Delete](#delete-resort-room-category-price)
below both reject `BAS`/`WKD`/`WKE` with `400 INVALID_ARGUMENT` (see [Error
Responses](#error-responses)). [Update](#update-resort-room-category-price) has no such restriction — every
price type, including `BAS`/`WKD`/`WKE`, can always be updated (amount, days, etc.) through this sub-resource.

`price_type_id`, `price_unit_id`, and `currency_id` are all **immutable after creation** — [Update Resort Room
Category Price](#update-resort-room-category-price) never changes them; to reclassify a `HOL`/`SPECIAL` price,
change its unit, or change its currency, delete it and create a new one (this escape hatch does not exist for
`BAS`/`WKD`/`WKE` — see above). All records support soft-delete — deleted records are hidden from all responses.
...
|--------|--------------------------------------------------------------------------------|------------------------------------------------------------------|
| POST   | `/api/v1/resorts/{resort-id}/room-categories/{room-category-id}/prices`      | Create a resort room category price                             |
...
| `price_type`           | Object  | —             | read-only; see [PriceType](price-types-api.md); resolved from `price_type_id`                    | Pricing rule classification (`BAS`/`WKD`/`WKE`/`HOL`/`SPECIAL`). Immutable after creation                  |
...
> `currency`) are write-only inputs, supplied only at creation — see [Create Resort Room Category
> Price](#create-resort-room-category-price) — and do not appear on this data model because the response always
> returns the resolved objects instead. Each embeds only `id`, `code`, `sort_order`, and the
...
Each `price_type` code additionally has its own shape requirements, enforced by database triggers (**not**
pre-validated by request-body bean validation, see the note under [Error
...
## Create Resort Room Category Price

`POST /api/v1/resorts/{resort-id}/room-categories/{room-category-id}/prices`

Creates a new pricing rule for the room category. **`price_type_id` must resolve to `HOL` or `SPECIAL`** —
`BAS`/`WKD`/`WKE` are rejected with `400 INVALID_ARGUMENT`, since those three are only ever created together
with the resort room category itself (see the intro above). `price_type_id` must reference an existing, active
[Price Type](price-types-api.md); `price_unit_id` an existing, active [Price Unit](price-units-api.md);
`currency_id` an existing, active [Currency](currencies-api.md) — an unknown id for any of the three returns
`404 ENTITY_NOT_FOUND`. Each id in `day_of_week_ids` must reference an existing, active [Day of
Week](days-of-week-api.md), or `404 ENTITY_NOT_FOUND`. `price_type_id`, `price_unit_id`, and `currency_id` are
all immutable after creation — see [Update Resort Room Category Price](#update-resort-room-category-price).

See [Price type rules](#price-type-rules) above for what `valid_from`/`valid_to`/`day_of_week_ids`/`priority`
must look like for each `price_type_id` — these shape rules are enforced by database triggers, not by request
validation.
...
| `price_unit_id`   | Long    | Yes      | Not null; must reference an existing, active price unit; immutable after creation                                                                            |
...
| `valid_from`      | Date    | —        | See [Price type rules](#price-type-rules); required for `HOL`/`SPECIAL`, forbidden otherwise                                                                 |
| `valid_to`        | Date    | —        | See [Price type rules](#price-type-rules); required for `HOL`/`SPECIAL`, forbidden otherwise                                                                 |
| `priority`        | Integer | —        | Nullable (defaults to `0`); forced to `0` server-side for `BAS`/`WKD`/`WKE` regardless of what's sent                                                        |
| `day_of_week_ids` | Long[]  | —        | See [Price type rules](#price-type-rules); required (non-empty) for `WKD`/`WKE`, forbidden otherwise; each id must reference an existing, active day of week |
...
restriction. `price_type_id`, `price_unit_id`, and `currency_id` are set at creation and cannot be changed — for
a `HOL`/`SPECIAL` row, reclassifying it, changing its unit, or changing its currency means deleting it and
creating a new one instead; `BAS`/`WKD`/`WKE` rows cannot be deleted or re-created at all, so their
`price_type_id`/`price_unit_id`/`currency_id` are effectively permanent.
...
|-------------|----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 400         | `INVALID_ARGUMENT`         | Missing or blank `Accept-Language` header (checked globally, before any endpoint runs — see the intro above); missing `currency-id` query parameter on [List](#list-resort-room-category-prices); missing/invalid required fields (`price_type_id`/`price_unit_id`/`currency_id`/`name`/`price` null on create); a `WKD`/`WKE` `price` greater than the room category's active `BAS` price in the same currency (checked at the application layer before the write); `price_type_id` resolving to `BAS`/`WKD`/`WKE` on [Create](#create-resort-room-category-price); the target row's `price_type` being `BAS`/`WKD`/`WKE` on [Delete](#delete-resort-room-category-price) (both checked at the application layer before the write)           |
| 404         | `ENTITY_NOT_FOUND`         | Resort not found; resort room category not found for the given `resort-id`/`room-category-id` pair (including a `room-category-id` that belongs to a different resort); price not found for the given `room-category-id`/`id` pair (including an `id` that belongs to a different room category); the price type, price unit, currency, or any `day_of_week_ids` entry not found; `currency-id` not found on [List Resort Room Category Prices](#list-resort-room-category-prices)                                                                                     |
| 409         | `CONFLICT`                 | The room category already has an active price with the same `price_type_id`/`price_unit_id`/`currency_id` combination (checked at the application layer before the write)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 500         | `INTERNAL_SERVER_ERROR`    | One of the [Price type rules](#price-type-rules) shape checks failed (e.g. `valid_from`/`valid_to` missing for `HOL`/`SPECIAL`, present for `BAS`/`WKD`/`WKE`; no day rows left for a `WKD`/`WKE` price; a day row attached to a non-`WKD`/`WKE` price; `price_type_id`/`price_unit_id` not assigned to the `ROOM_CATEGORY` price scope). These rules are enforced only by database triggers, not by request-body validation, so a request that violates them is accepted past validation and fails at the database layer — the trigger's raised exception isn't a recognized constraint-violation type, so it falls through to the generic error handler as a `500` rather than a `400`/`409` |
