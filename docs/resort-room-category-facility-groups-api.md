...
is always `codes.length`. Use this to gray out/disable locales already present in `codes` when building the
picker for [Create Resort Room Category Facility Group
Locale](#create-resort-room-category-facility-group-locale) — `locale_id` must not already have a translation
for this resort room category facility group, or the create call returns `409 CONFLICT`.
