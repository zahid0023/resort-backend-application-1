...
Room categories classify rooms by tier or type (e.g., Standard, Deluxe, Suite). Category names and descriptions are locale-specific and are embedded in every response via the `locales` array. All records support soft-delete — deleted records are hidden from all responses.
...
Creates a room category along with its locale-specific translations in one request. All provided `locale_id` values must reference existing, active locales.
...
Updates `sort_order`. The `code` field is set at creation time and cannot be changed. Locale translations are managed via the room category locale endpoints.
...
Room category locale endpoints manage per-locale translations for a room category. The `{room-category-id}` path parameter must reference an existing, active room category.
...
Soft-deletes a room category locale. The record is not removed from the database but will no longer appear in any response.
